...
-- depth 5
--
local TEST_MAX_DEPTH =
    5


local TEST_MIN_BRANCHES =
    1


local TEST_MAX_BRANCHES =
    4


local DUNGEON_POOL_SIZE =
    5


-- TEMPORARY:
--
-- Keep this at 1.0 for the first lobby test.
--
-- That guarantees:
--
-- Regular -> Lobby -> Regular -> Lobby...
--
-- Once everything works, change this to:
--
-- 0.15
--
local LOBBY_ROOM_CHANCE =
    1.0
...
local function push_history(
    ...
    -- Prevent duplicate pushes if the warp collision fires
    -- more than once before the transfer finishes.
    local previous =
        history[
            #history
        ]
    ...
    if previous
        and previous.area_id ==
            area_id
        and previous.object_id ==
            object_id
    then
        return
    end
...
local function generate_room(
    ...
    -- ----------------------------------------------------------
    -- Assign actual runtime identity
    -- ----------------------------------------------------------
    ...
    -- ----------------------------------------------------------
    -- Create NPC bots for the dynamically-loaded area
    -- ----------------------------------------------------------
    --
    -- The map did not exist during ezlibs startup, so explicitly
    -- scan its NPC placeholders now.
    -- ----------------------------------------------------------
    ...
    -- ----------------------------------------------------------
    -- Remove claimed cache file
    -- ----------------------------------------------------------
    --
    -- Net.update_area() has already parsed the map.
    -- The generator will notice the missing pool slot and
    -- generate a replacement in the background.
    -- ----------------------------------------------------------
...
Net:on(
    ...
    function(event)
        -- Moving from one dungeon room to another leaves at
        -- least one player in active_run.areas.
        --
        -- Returning the final player to the overworld destroys
        -- the shared runtime dungeon.
